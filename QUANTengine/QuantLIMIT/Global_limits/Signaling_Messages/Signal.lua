...
local nameEventCache = QuantCORE():NamedEventManager();
local iCount = 0;
local strings;
...
function OnTimer( id )
	if( id == 200 ) then
		if( iCount ~= 10 ) then
			...
			nameEventCache:SendSignal( "signalingCase02", "key02",   RS_STRING_SIGNAL,  strings );
			nameEventCache:SendSignal( "signalingCase03", "key03",   RS_STRING_SIGNAL,  strings );
			nameEventCache:SendSignal( "signalingCase04", "key04",   RS_STRING_SIGNAL,  strings );
			iCount = iCount + 1;
		end
	end
end
